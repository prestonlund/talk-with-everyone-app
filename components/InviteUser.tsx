    ...
    async function onSubmit(values: z.infer<typeof formSchema>) {
        ...
        if (isPro && noOfUsersInChat >= 2) {
            toast({
                ...
                description: "You have exceeded the limit of users in a single chat for the PRO plan.",
                variant: "destructive",
                action: (
                <ToastAction altText="Upgrade" onClick={() => router.push("/register")}>
                    Upgrade to PRO
        ...
        if (querySnapshot.empty) {
            ...
            const chatMembersSnapshot = await getDocs(chatMembersRef(chatId));
            const userAlreadyInChat = chatMembersSnapshot.docs.some(doc => doc.data().userId === user.id);
            ...
            await setDoc(addChatRef(chatId, user.id), {
                ...
                toast({
                    title: 'Added to chat',
                    description: 'The user has been aded to the chat successfully',
